...
//
// **This works on lines, not on elements.** Walking the document's top-level
// children and asking whether each one is a fence makes the answer depend on
// how Word or Google Docs happened to nest things — the same lesson imports or
// fails depending on whether the author used Enter or Shift+Enter, a bullet
// list, or the grey code box. So the document is flattened to a list of lines
...
// `@tiptap/extension-code-block` parses — language read off the class. So
// nothing here touches the document schema.
...
/** The raw language token on a fence line, or null when the line is not a fence. */
function fenceToken(line: string): string | null {
  const match = isOpenFence(line.trim());
  return match ? (match[1] ?? "") : null;
}

function resolve(raw: string) {
  const id = normalizeLang(raw);
  const recognised =
...
export function extractCodeBlocks(html: string): CodeBlockResult {
  ...
  const fenceIn = (unit: Unit, from: number): number =>
    unit.lines.findIndex(
      (line, idx) => idx >= from && fenceToken(line) !== null,
    );
  ...
  while (u < units.length) {
    ...
    // end — it invents no closing fence, changes nothing, and says so. Running
    // the block to the next heading instead turns one missing ``` into a lesson
    // full of code that was never code.
    if (endU === -1) {
      const opened = fenceToken(start.lines[openLine]) ?? "";
      warnings.push(
    ...
    // cleanCodeText undoes the curly quotes and exotic spaces a word processor
    // silently applies — they compile to nothing and are invisible on screen.
    const codeText = cleanCodeText(code.join("\n").replace(/^\n+|\n+$/g, ""));

    const languageId = record(fenceToken(start.lines[openLine]) ?? "");

    // Rebuild: anything that shared an element with a fence comes back as plain
    // paragraphs. It loses bold and links, which only happens when an author put
    // prose on the same line as a fence or inside a code box — far better than
    // leaving it buried in the code block.
    ...
    // not finished with. A document with several blocks in one code box puts the
    // *next* block's opening fence in exactly this leftover; skipping it orphans
    // that block and leaves every later fence paired one step out. `leading`
    // needs no such treatment: it sits before the first fence in the element, so
    // by definition it contains none.
    units.splice(
      lastU + 1,
      0,
