...
const Overview: React.FC<OverviewProps> = ({
  ...
  const [canAcceptResponse, setCanAcceptResponse] = useState(events.canAcceptResponse);
  ...
  const handleCopy = async () => {
    try {
      ...
      console.log("Could not copy automatically. Please copy it manually:\n" + FORM_LINK);
    }
  };
        ...
        <div className="flex flex-col justify-between gap-5 flex-1 p-4">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        <a
                          ...
                          className="text-[16px] text-blue-600 font-medium hover:underline"
                        >
                          {data.eventType === "Workshop" ? "Feedback Form Link" : "Submission Form Link"}
                        </a>

                        <Button variant={"ghost"} disabled={copied} className="cursor-pointer" onClick={handleCopy}>{copied ? <CopyCheck size={18} /> : <Copy size={18} />}</Button>
                      </div>
                    </div>
                  )}
                  
                  <AcceptResponseToggle eventId={data.id} initialValue={data.canAcceptResponse} onToggle={handleToggleFeedback} />
          ...
          <div className="flex gap-3">
            <Button
              ...
              onClick={(e) => {
                router.push(`/${role}/events/edit/${events.id}`);
              }}
            >
