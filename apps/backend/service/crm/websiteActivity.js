...
const isEnabled = () =>
  process.env.CRM_WEBSITE_VISIT_ENABLED === "true" &&
  !!process.env.CRM_URL &&
  !!process.env.CRM_WEBSITE_VISIT_SECRET;
...
async function sendActivityBatch(rows) {
  ...
  const signature = crypto
    .createHmac("sha256", process.env.CRM_WEBSITE_VISIT_SECRET)
    .update(body)
    .digest("hex");

  const res = await axios.post(
    `${process.env.CRM_URL.replace(/\/+$/, "")}/api/v1/webhooks/website-activity`,
    body,
    {
      headers: {
