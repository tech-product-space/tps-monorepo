 ...
 *
 * Full design in `../RECORDINGS_PLAN.md`. Three things about it are easy to get
 * wrong from elsewhere:
 *
 * - **`video.url` is not public while `settings.gateVideo` is on.** The public
 *   detail controller strips it. A YouTube link is not a secret and we are not
...
export default (sequelize) => {
  const Recording = sequelize.define(
    "Recording",
    {
       ...
       * logos. An array even though both designs show one — `speakers[0]` also
       ...
       * `Event.numberOfAttendees` — it is the live session's attendance, which
      ...
      /** `{ subject, body }` — the "here's your link" email. */
      seo: {
        type: DataTypes.JSONB,
        allowNull: true,
        defaultValue: {},
       ...
       *
       * Stored, and validated against the same `EVENT_TYPES` the event form
       * uses. It replaced a foreign key to `Events`: the link existed only to
       * derive this, cost a join on every public read, and could not describe a
       * recording that never had a live session behind it.
       */
      format: {
        type: DataTypes.STRING,
        allowNull: true,
