...
/**
 * The chip row on /recordings — "Product Strategy", "AI for PMs", …
 *
 * A managed catalogue rather than a free-text column on `Recordings`: the chips
 * have a deliberate left-to-right order, the selected chip belongs in the URL so
 * it needs a stable slug, and the set is curated — with free text one typo
 * ("Prodct Strategy") silently publishes an extra chip.
 */
module.exports = (sequelize, DataTypes) => {
  const RecordingCategory = sequelize.define(
    "RecordingCategory",
    {
      ...
      /** Chip label — "Product Analytics". */
      name: {
        type: DataTypes.STRING,
        allowNull: false,
      },

      /** Goes in the URL: `/recordings?category=product-analytics`. */
      slug: {
        type: DataTypes.STRING,
        allowNull: false,
        unique: true,
