...
const ATTENDED_STATUS_ID = psEnv.MEETING_ATTENDED_STATUS_ID || null;
const ATTENDED_LABEL = (
  psEnv.MEETING_ATTENDED_STATUS_LABEL || "Mentor Call Done"
)
  .trim()
  .toLowerCase();
const NOSHOW_STATUS_ID = psEnv.MEETING_NOSHOW_STATUS_ID || null;
const NOSHOW_LABEL = (
  psEnv.MEETING_NOSHOW_STATUS_LABEL || "Mentor Call Not Attended"
)
  .trim()
  .toLowerCase();
...
const CANCELLED_LATE_STATUS_ID =
  psEnv.MEETING_CANCELLED_LATE_STATUS_ID || null;
const CANCELLED_LATE_LABEL = (
  psEnv.MEETING_CANCELLED_LATE_STATUS_LABEL || "Mentor Call Cancelled"
)
  .trim()
  .toLowerCase();

// How long a meeting may sit unlogged before the queue calls it stale.
const STALE_OUTCOME_DAYS = Number(psEnv.MEETING_STALE_OUTCOME_DAYS || 3);

// Status applied to the lead when a status-synced meeting is cancelled.
const CANCEL_STATUS_ID = psEnv.MEETING_CANCEL_STATUS_ID || null;
const CANCEL_LABEL = (
  psEnv.MEETING_CANCEL_STATUS_LABEL || "Mentor Call Cancelled"
)
  .trim()
  .toLowerCase();
