 ...
 */
// In the monorepo this app is mounted under /gradient. The activity registry and
// its prefix fallback are keyed on the app-relative path ("/blogs/..."), so the
// mount prefix has to come off or every lookup misses.
const stripMount = (p) => p.replace(/^\/gradient(?=\/|$)/, "");
...
export const activityLogger = (req, res, next) => {
  ...
  res.on("finish", () => {
    try {
      ...
      const routeKey = `${req.method} ${stripMount(req.baseUrl || "")}${
        req.route?.path || ""
      }`;
      ...
      const { descriptor, matched } = resolveActivity(
        routeKey,
        req.method,
        stripMount(req.originalUrl.split("?")[0]),
      );
      ...
      const payload = {
        ...
        metadata,
        method: req.method,
        path: stripMount(req.originalUrl.split("?")[0]),
        routeKey,
