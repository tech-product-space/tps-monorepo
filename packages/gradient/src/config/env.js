...
export default {
  ...
  postgres: {
    host: psEnv.POSTGRES_HOST,
    port: psEnv.POSTGRES_PORT,
    database: psEnv.POSTGRES_DB,
    username: psEnv.POSTGRES_USER,
    password: psEnv.POSTGRES_PASSWORD,
  },
  ...
  crm: {
     ...
     */
    baseUrl: psEnv.CRM_URL || null,
  },

  assets: {
    /** CDN domain in front of the S3 bucket, matching the websites' storage helper. */
    baseUrl: psEnv.AWS_FILE_BASE_URL || "https://assets.thegradient.co.in",
  },
  ...
  s3: {
     ...
     */
    privateBucket: psEnv.AWS_PRIVATE_BUCKET_NAME || null,
  },
   ...
   */
  publicSiteUrl: psEnv.PUBLIC_SITE_URL || "https://gradientlearnings.org",

  google: {
    auth: {
      clientId: psEnv.GOOGLE_AUTH_CLIENT_ID,
      clientSecret: psEnv.GOOGLE_AUTH_CLIENT_SECRET
    }
  },

  agendaEnabled: psEnv.AGENDA_JOBS_ENABLED === "true" || false,
   ...
   */
  tokenEncryptionKey: psEnv.TOKEN_ENCRYPTION_KEY || null,
   ...
   */
  adminSiteUrl: psEnv.ADMIN_SITE_URL || "https://admin.gradientlearnings.org",
  ...
  preview: {
    ...
    launchTtl: psEnv.PREVIEW_LAUNCH_TTL || "15m",

    /** In an httpOnly cookie, never in a URL — one editing session's worth. */
    sessionTtl: psEnv.PREVIEW_SESSION_TTL || "60m",
  },
  ...
  meta: {
     ...
     */
    enabled: psEnv.META_INTEGRATION_ENABLED === "true" || false,

    /** A Graph version bump should be a config change, not a code edit. */
    graphVersion: psEnv.META_GRAPH_VERSION || "v22.0",

    /** Where the token-expiry alert goes. Null disables the alert. */
    alertEmail: psEnv.META_ALERT_EMAIL || null,
  },
  ...
  workflows: {
     ...
     */
    enabled: psEnv.WORKFLOWS_ENABLED === "true" || false,

    redis: {
      /** One URL, or the parts below. `REDIS_URL` wins when both are set. */
      url: psEnv.REDIS_URL || null,
      host: psEnv.REDIS_HOST || "127.0.0.1",
      port: Number(psEnv.REDIS_PORT) || 6379,
      password: psEnv.REDIS_PASSWORD || undefined,
      /** Managed Redis usually requires it; a local one usually refuses it. */
      tls: psEnv.REDIS_TLS === "true" || false,
    },
     ...
     */
    queuePrefix:
      psEnv.WORKFLOW_QUEUE_PREFIX ||
      `gradient:${psEnv.NODE_ENV || "development"}`,

    /** Per-queue worker concurrency. Kept low against the SES send rate. */
    concurrency: Number(psEnv.WORKFLOW_CONCURRENCY) || 5,
  },
};
