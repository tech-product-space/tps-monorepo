...
import {
  ...
  loadGateUser,
  resolveWatchProfile,
} from "../../services/recording/watchProfile.service.js";
 ...
 *
 * Two endpoints, both behind `authMiddleware`, because watching a gated
 * recording now requires an account:
 *
 ...
 *
 * ── Why the form is not asked for every time ────────────────────────────────
 *
 * The form is ten fields. Somebody working through four recordings used to
 * fill it four times, which is the surest way to lose them halfway. So the
 * first pass asks, and later recordings **carry** the answers forward: one
 * click, a real lead row for the new recording, no form.
 *
 * Carried details go stale — people graduate, change jobs, move companies — so
 * the carry expires. `isRecordingProfileStale` holds both rules and the reason
 * each exists.
 *
 * **A repeat submission is a 200, not a 409.** Unchanged. Somebody confirming
 * their details has done nothing wrong, the row is updated rather than
 * inserted, and that is what keeps `RecordingLeads` a list of people rather
 * than a list of page loads — and what stops the lead event and the workflow
 * trigger firing again, since both hang off `afterCreate`.
 */

/** What the form's attendee toggle may say. Anything else is not stored. */
const ATTENDEE_TYPES = ["Professional", "Student"];
 ...
 * loads — the same reason `visitorDetails` is a prefill and never a submission.
 * The one write it does make is repair: claiming an unclaimed lead row for the
 * account whose address it carries, which changes nothing anybody can observe.
 *
 ...
 */
export const getWatchState = asyncWrapper(async (req, res) => {
  const recording = await loadWatchableRecording(req.query.recordingId, res);
  if (!recording) return undefined;

  const user = await loadGateUser(req.user.id);
 ...
 *
 * `name` and `email` are taken from the body when it has them, falling back to
 * the account. Both are editable in the form — a personal sign-up address is
 * often not the one somebody wants a course team writing to, and a Google
 * display name is often not the name they want on a lead.
 *
 ...
 */
export const createRecordingLead = asyncWrapper(async (req, res) => {
  ...
  const {
    ...
    // `jobTitle` column that was already here. This line is the whole mapping.
    role,
    jobTitle,
    attendeeType,
    collegeName,
    ...
    additionalData,
    /**
     * "Update my details" rather than "let me in" — the link under a player
     * somebody is already watching. It refreshes the details and restarts the
     * freshness window without counting as another view, because they are not
   ...
   *
   * `EventDetailsForm` already refuses to submit without a name, a valid phone
   * for the chosen country, and the role-or-college pair that the attendee
   * toggle makes mandatory. Re-deriving that branch server-side would be a
   * second copy of a rule that changes, and the two would drift.
   *
   ...
   */
  if (attendeeType && !ATTENDEE_TYPES.includes(attendeeType)) {
  ...
  if (!submitted && existing) {
    return res.status(200).json({
      ...
      data: {
        video: videoPayload(recording),
        lead: { id: existing.id, detailsConfirmedAt: existing.detailsConfirmedAt },
      },
    });
  }
   ...
   *
   * This inverts the old "only fill gaps" merge, which existed for a gate that
   * reappeared on every new device and could be re-submitted half-empty by
   * somebody in a hurry. Every submission is now a deliberate act by a
   * signed-in person against a prefilled form — so when they change an answer,
   * changing it is the entire point, and refusing to overwrite would make the
   * confirmation cosmetic. The only field that can go from set to null this way
   * is `linkedinUrl`; the rest are required by the form.
   */
  /**
  ...
  const resolvedEmail = submitted
    ? (trimmed(email)?.toLowerCase() ?? user.email)
  ...
  const details = submitted
    ? {
        ...
        // contradicts their own attendee type.
  ...
  try {
    if (existing) {
      ...
      lead = await RecordingLead.create({
         ...
         * arrived at *this* recording; copying August's campaign parameters onto
         * a March visit would credit a campaign that had already ended.
         */
     ...
     *
     * Named rather than left to the generic 409 the error middleware would
     * give, because "that email is already registered" is actionable and
     * "duplicate key value violates unique constraint" is not. It is not a
     * security answer either way: they are told nothing they did not just type.
     */
  ...
  /**
   * Counts watch starts. Not bumped by `confirmOnly`, which is somebody
   * updating their details under a player they are already watching, and not
   * bumped by the probe — so this stays "how many times was this watched"
   * rather than "how many times was this page opened".
   */
  if (!confirmOnly) await recording.increment("viewCount");

  // No watch-link email. The gate hands the video back in this response and the
  // player starts immediately, so a mail repeating the link arrived after the
  // person was already watching — it was a second copy of something they had,
  // not a delivery mechanism. Removed along with the template editor rather
  // than left switched off, so there is no dormant mailer to rediscover.
...
export const listRecordingLeads = asyncWrapper(async (req, res) => {
  ...
  // Partial, case-insensitive. This backs a search box, and an exact match is
  // no use in one: nobody types a whole address to find a row they are already
  ...
  const { rows, count } = await RecordingLead.findAndCountAll({
    where,
    include: [
      { model: Recording, as: "recording", attributes: ["id", "title", "slug"] },
    ],
