...
const run = async () => {
  ...
  try {
    ...
    const unsigned = `https://${psEnv.AWS_BUCKET_NAME}.s3.${psEnv.AWS_REGION}.amazonaws.com/${issued[0].fileKey}`;
    ...
    for (const key of [backgroundKey, ...createdFileKeys].filter(Boolean)) {
      await s3
        .send(new DeleteObjectCommand({ Bucket: psEnv.AWS_BUCKET_NAME, Key: key }))
        .catch(() => {});
    }
