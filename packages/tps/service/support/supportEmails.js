...
function ticketUrl(ticket) {
  const base = (psEnv.FRONTEND_URL || "https://www.theproductspace.co.in").replace(/\/$/, "");
...
function send(to, subject, html, ticket, eventType) {
  ...
  sendEmail({
    ...
    meta: {
      ...
      sourceId: ticket?.id ? String(ticket.id) : null,
      sourceName: ticket?.ticket_number ? `Ticket #${ticket.ticket_number}` : "Support Ticket",
      extra: { eventType, ticketSubject: ticket?.subject },
    },
  }).catch((e) =>
...
function sendTicketCreated(ticket) {
  ...
  send(ticket.requester_email, `We've received your query (${ticket.ticket_number})`, html, ticket, "created");
}
...
function sendNewReply(ticket) {
  ...
  send(ticket.requester_email, `New response on your query (${ticket.ticket_number})`, html, ticket, "reply");
}
...
function sendTicketClosed(ticket) {
  ...
  send(ticket.requester_email, `Your query has been closed (${ticket.ticket_number})`, html, ticket, "closed");
}
