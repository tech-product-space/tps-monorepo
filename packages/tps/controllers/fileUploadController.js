...
exports.uploadAiProductFile = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            key,
            fileUrl: `https://${psEnv.AWS_BUCKET_NAME}.s3.${psEnv.AWS_REGION}.amazonaws.com/${key}`,
        });
    } catch (error) {
...
const publicUrlFor = (key) =>
    `https://${psEnv.AWS_BUCKET_NAME}.s3.${psEnv.AWS_REGION}.amazonaws.com/${key}`;
...
exports.uploadFile = async (req, res) => {
    try {
        ...
        const fileUrl = `${psEnv.AWS_PROD_BUCKET_URL}/${fileName}`;
...
exports.uploadBlogsFile = async (req, res) => {
    try {
        ...
        const fileUrl = `${psEnv.AWS_BLOG_BUCKET_URL}/${fileName}`;
    ...
    try {
        ...
        const files = data.Contents.map(file => {
            ...
            return {
                ...
                url: `${psEnv.AWS_BLOG_BUCKET_URL}/${keyWithoutPrefix}`,
                size: file.Size,
                lastModified: file.LastModified,
            };
        });
...
exports.uploadResumeFile = async (req, res) => {
    try {
        ...
        const fileUrl = `${psEnv.AWS_RESUME_BUCKET_URL}/${fileName}`;
...
exports.getAllResumeFiles = async (req, res) => {
    try {
        ...
        const files = data.Contents.map(file => {
            ...
            return {
                key: keyWithoutPrefix,
                url: `${psEnv.AWS_RESUME_BUCKET_URL}/${keyWithoutPrefix}`,
                size: file.Size,
                lastModified: file.LastModified,
            };
        });
...
exports.uploadProjectFile = async (req, res) => {
    try {
        ...
        const fileUrl = `${psEnv.AWS_PROJECT_BUCKET_URL}/${fileName}`;
...
exports.uploadEventFile = async (req, res) => {
    try {
        ...
        const fileUrl = `${psEnv.AWS_EVENT_BUCKET_URL}/${fileName}`;
...
const publicUrlForKey = (key) => {
    const base = (
        psEnv.AWS_ASSETS_CLOUDFRONT_DOMAIN ||
        `https://${psEnv.AWS_BUCKET_NAME}.s3.${psEnv.AWS_REGION}.amazonaws.com`
    ).replace(/\/$/, "");
